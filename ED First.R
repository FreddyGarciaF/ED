...
NP <- 5 #numero poblacion en vectores
dimension <- 2
LB <- rep(-5,dimension) #columnas
UB <- rep(5,dimension) #valor maximo
...
for ( G in 1:30 ){ #N veces
  ...
  for (z in 1:NP ) { # Inicio de las iteraciones
        ...
        ifelse( fobj(vu) < fobj(vpadre),  pob_ini[z,] <- vu , pob_ini[z,]  <- vpadre )
  }
   BestObj <- rbind(BestObj, best_obj(pob_ini))
}
